...
#define SIGNAL_TIMEOUT_MS 2000U // 2 seconds LIN中斷訊號，POS亮
...
static uint8_t LIN_timeout = 10;                       // 超過10ms(預設值)沒有接收資料，視為接收失敗
static bool LIN_timerRunning = false;                  // LIN 定時器運作中
static volatile uint8_t CountCallBack = 0;             // 10ms 超時計時器（毫秒）
static volatile uint16_t LIN_signalElapsedMs = 0;      // LIN訊號已用時間(計時器)（毫秒）
static volatile bool LIN_validSignalReceived = false;  // LIN 有效訊號已接收
static volatile bool LIN_signalTimedOut = false;       // LIN 訊號超時
static volatile bool LIN_signalTimeoutPending = false; // LIN 訊號超時待處理

bool LIN_signalTimeoutOccurred(void) {
  if (LIN_signalTimeoutPending) { //LIN 訊號超時待處理
    LIN_signalTimeoutPending = false;
    return true;
  }
...
{ // TMR0 interrupt handler 1ms

    if (LIN_validSignalReceived)
    { // LIN 有效訊號已接收
        LIN_validSignalReceived = false;
        LIN_signalElapsedMs = 0;
        LIN_signalTimedOut = false;
        LIN_signalTimeoutPending = false;
    }
    else if (!LIN_signalTimedOut)
    {
        if (++LIN_signalElapsedMs >= SIGNAL_TIMEOUT_MS)
        {
            LIN_signalTimedOut = true;
            LIN_signalTimeoutPending = true;
        }
    }
